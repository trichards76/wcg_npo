...
define([
	...
	Toolbar, has,
	MapUrlParams, Growler,

	arcgisUtils, esriLang, urlUtils,
	FeatureLayer, Query,

	QueryTask,webMercatorUtils,Extent,
		...
		return declare(null, {
			...
			startup: function (config) {
				...
				if (config) {
					...
					if (this.config.dsd_tab != null || this.config.dsd_json != null) {
						//can add zoom to functions here if required later
						var queryUrl = this.config.dsdServices.dsdnpoSubplaces;
						var queryTaskFilterSub = new QueryTask(queryUrl);
						var queryFilterSub = new esri.tasks.Query();
						queryFilterSub.returnGeometry = true;
						queryFilterSub.outFields = ['*'];
						queryFilterSub.outSpatialReference = { wkid: 102100 };
						queryFilterSub.where = "PR_NAME = 'Western Cape'";
						queryTaskFilterSub.execute(queryFilterSub, lang.hitch(this, '_setDefaultHomeZoom'));
					}
				} else {
			...
			// Map is ready
			_mapLoaded: function () {
				//If a suburb is provided, query the suburb layer for matching suburb polygon and zoom to it
				if (this.config.dsd_tab != null || this.config.dsd_json != null) {
					var queryUrl = this.config.dsdServices.dsdnpoSubplaces;
					var queryTaskFilterSub = new QueryTask(queryUrl);
					var queryFilterSub = new esri.tasks.Query();
					queryFilterSub.returnGeometry = true;
					queryFilterSub.outFields = ['*'];
					queryFilterSub.outSpatialReference = { wkid: this.map.spatialReference };
					queryFilterSub.where = "PR_NAME = 'Western Cape'";
					queryTaskFilterSub.execute(queryFilterSub, lang.hitch(this, '_zoomToBoundary'));
				}
			...
			_zoomToBoundary: function (results) {
				var extent = new Extent(results.features[0].geometry.getExtent());
				...
				//this.config.itemInfo.item.extent = extent.expand(1.2);
				
				// remove loading class from body
				domClass.remove(document.body, "app-loading");
			},
			// Set the Default Home Zoom if a Suburb was Sent
			_setDefaultHomeZoom: function (results) {
				var extent = new Extent(results.features[0].geometry.getExtent());
				...
				var expandedExtent = extent.expand(1.2);
				this.config.itemInfo.item.extent = [webMercatorUtils.xyToLngLat(expandedExtent.xmin, expandedExtent.ymin),webMercatorUtils.xyToLngLat(expandedExtent.xmax,expandedExtent.ymax)];
					...
					for (var i = 0; i < this.config.tools.length; i++) {
						switch (this.config.tools[i].name) {
							...
							case "share":
								...
								break;
							case "Editor":
								toolList.push(this._adddsdTool(this.config.tools[i],
									toolbar, "medium"));
								break;
							default:
								break;
			...
			_adddsdTool: function (tool, toolbar, panelClass) {
				var deferred = new Deferred();

				if (has("Editor")) {
					require(["application/dsdTool"], lang.hitch(this, function (dsdTool) {
						var canUseTool = true;
						if (this.config.dsd_tab == null || this.config.dsd_json == null) {
							var myDialog = new Dialog({
								title: "Warning:",
								content: "The Required Parameters have not been Provided! The Query Tool functionality will not be Available!",
								style: "width: 500px"
							});
						...
						//Also check that the urls have been provided for the dsdServices (i.e. endpoints for the suburb, street and police precincts and stations
						if (this.config.dsdServices.dsdnpoSubplaces == null) {
							var myDialog = new Dialog({
								title: "Warning:",
								content: "The Required Data Services were not Found! The Query Tool functionality will not be Available!",
								style: "width: 500px"
							});
						...
						//query the boundary service to see if a poly for the dsdid exists and is active (Verified/Unverified)
						//if it exists and the action is create, throw an error and do not instantiate
						//display alert message if it exists
						
						//editied: jfeeke: 15/10/2018 - change query based on parameter as Create OR Edit
						var queryString = "1=1"//;"dsd_ID = '" + this.config.dsd_UniqueID + "' AND (STATUS = " + 1 + ")";
						//if (this.config.dsd_Action == "Edit") {queryString = "dsd_ID = '" + this.config.dsd_UniqueID + "' AND (STATUS = " + 1 + ")";}
						
						if (canUseTool == true) {
							var queryUrl = this.config.dsdServices.dsdnpoSubplaces;
							var queryTaskFilterID = new QueryTask(queryUrl);
							var queryFilterID = new esri.tasks.Query();
							queryFilterID.returnGeometry = false;
...
/* 									if (returnResults.results[0].features.length >= 1)
									...
									{ */
										...
										var dsdDiv = toolbar.createTool(tool, panelClass);
										this.dsdTool = new dsdTool(
											{
												map: this.map,
												summary: "",
												title: "DSD NPO Tool",
												...
												//dsdSuburb: this.config.dsd_Suburb,
												//dsdTown: this.config.dsd_Town,
												dsdTab: this.config.dsd_tab,
												dsdJson: this.config.dsd_json,
												defaultSymbols: this.config.defaultSymbols,
												//dsdAction: this.config.dsd_Action,
												msgGrowler: growler
											}, dsdDiv);
										domClass.add(this.dsdTool.domNode, "pageBody");
										this.dsdTool.startup();
										deferred.resolve(true);
/* 									} */
								})
							);
			...
			_checkdsdBoundaryStatus: function (results) {
				if (results.features.length == 0) {
					...
					//editied: jfeeke: 15/10/2018 - not required any more. should always be a valid parameter received
/* 					if (this.config.dsd_Action == "Create") {
						var myDialog = new Dialog({
							title: "Warning:",
							content: "The Generated Unique ID already exists in the System. Please Close this Map and try again! The Edit Tool functionality will not be Available!",
							style: "width: 500px"
						});
